  ...
  return (
    <div className="sheet-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className={`detail-sheet ${dish.photo ? 'has-photo' : ''}`} role="dialog" aria-modal="true" aria-label={dish.name}>
        {dish.photo && <div className={`detail-photo ${dish.category === 'wine' ? 'wine-photo' : ''}`}>
        ...
        <div className="detail-body">
          {dish.category === 'wine' ? <><p className="eyebrow">О вине</p><dl className="wine-facts">{Object.entries(dish.components).map(([label, values]) => <div key={label}><dt>{label}</dt><dd>{values.join(', ')}</dd></div>)}</dl></> : dish.recipe ? <RecipeCard key={dish.id} recipe={dish.recipe} /> : <><p className="eyebrow">{dish.category === 'sauces' ? 'Состав' : 'Основа и дополнения'}</p>
          ...
          </>}{dish.category !== 'wine' && dish.category !== 'sauces' && dish.service_note && <div className="info-block"><span className="info-symbol">!</span><div><strong>{dish.recipe ? 'Приготовление и подача' : 'Важно для гостя'}</strong><p>{dish.service_note}</p></div></div>}
          {dish.category !== 'wine' && <div className="allergen-row"><span>Аллергены</span><strong>{dish.allergens.join(', ') || (dish.category === 'sauces' ? 'не указаны' : 'не указаны — уточнить состав')}</strong></div>}
          {dish.category !== 'wine' && dish.category !== 'sauces' && <p className="field-help">Список составлен по доступной техкарте. При аллергии у гостя уточните состав и возможный перекрёстный контакт на кухне.</p>}
...
function MenuView({ dishes, onSelect }: { dishes: Dish[]; onSelect: (dish: Dish) => void }) {
  ...
  const renderDishList = (items: Dish[]) => <div className="dish-list">{items.map((dish, index) => <button className="dish-card" key={dish.id} onClick={() => onSelect(dish)}><div className={`dish-visual ${dish.color} ${dish.category === 'wine' ? 'wine-thumbnail' : ''}`}>{dish.photo ? <img src={/^\/dishes\/[a-z0-9-]+-960\.webp$/.test(dish.photo) ? dish.photo.replace('-960.webp', '-480.webp') : dish.photo} alt="" width={96} height={96} loading="lazy" decoding="async" /> : <><span>{String(index + 1).padStart(2, '0')}</span><i /></>}</div><div className="dish-copy"><div className="dish-meta"><span className="tag">{dish.badge}</span>{dish.recipe && <span className="weight-chip">{dish.recipe.variants.length > 1 ? dish.recipe.variants.map((v) => v.label).join(' / ') : 'Техкарта'}</span>}{dish.weight > 0 && <span className="weight-chip">{dish.weight} г</span>}</div><h3>{dish.name}</h3><p>{dish.category === 'wine' ? [dish.components['Происхождение']?.join(', '), dish.components['Стиль']?.join(', ')].filter(Boolean).join(' · ') : dish.ingredients.slice(0, 4).join(', ')}</p></div><span className="chevron">›</span></button>)}</div>;
  ...
  if (!currentSection) return <>
    ...
    <div className="category-grid">
      {menuSections.map((section, index) => <button className={`category-card tone-${section.tone}`} key={section.id} onClick={() => { catalogScrollTop.current = menuRef.current?.closest<HTMLElement>('.content-scroll')?.scrollTop ?? 0; setSectionId(section.id); setWineServing('Бокалы'); setSearchOpen(false); setQuery(''); }}>
  ...
  return <section ref={menuRef} className="category-view">
    ...
    {currentSection.id === 'wine' ? <>
      <div className="wine-serving" aria-label="Подача вина">{['Бокалы', 'Бутылки'].map((serving) => <button key={serving} aria-pressed={wineServing === serving} onClick={() => setWineServing(serving)}>{serving}</button>)}</div>
      {wineServing === 'Бутылки' ? <div className="empty-state"><strong>Вина в бутылках</strong><p>Позиции скоро появятся.</p></div> : ['Красное', 'Белое', 'Игристое'].map((group) => {
        const wines = sectionDishes.filter((dish) => dish.components['Подача']?.includes('Бокалы') && dish.components['Тип']?.includes(group));
        return <section className="dish-subsection" key={group}><div className="dish-group-heading"><div><strong>{group}</strong><small>{wines.length} позиций</small></div></div>{renderDishList(wines)}</section>;
      })}
    </> : hasDishes ? <>
...
export default function Home() {
  ...
  const menuDishes = useMemo(() => [...data.dishes, ...buildSauces(data.dishes)], [data.dishes]);
